...
for i=1:size(COORD,2)
    ...
    x=COORD(:,i);
    if (  (  ( (x(1)-0.5)^2  + (x(2)-0.5)^2  -0.0625 )^3  -(x(1)-0.5)^2 * (x(2)-0.5)^3 ) <=0)  %%%**<=0;//,=h1/3 to consider the nodes outside the domain nearer to the boundary
         FCOORD(:,i)=COORD(:,i);
     else FCOORD(:,i)=[0,0];
    end
...
for i=1:size(INTLNODS,2)  
       if  (  ((x_G(1,i)-0.5)^2 + (x_G(2,i)-0.5)^2 -0.0625)^3  -(x_G(1,i)-0.5)^2*(x_G(2,i)-0.5)^3 <=0)
...
% to find the nodes which lie completely inside the approximated(by COG
% approach) domain \omega
% CINRNOD=zeros(size(INRNOD,1),1);
% 
% for i=1:size(INRNOD,1)
%     if ( (COORD1(1,INRNOD(i))-0.5)^2 + (COORD1(2,INRNOD(i))-0.5)^2 - ((COORD1(1,INRNOD(i))-0.5)^(2/3))*(COORD1(2,INRNOD(i))-0.5)<=((1/16)-(h1/3)) )
%     end
% end
% CINRNOD=nonzeros(CINRNOD);
%% NODES WHICH LIE near but on/outside BOUNDARy%%
% GMANOD=setdiff(INRNOD,CINRNOD);

%% Boundary Nodes
% V1=setdiff(INTLNODS(1,:),ILNODS(1,:));
% V2=setdiff(INTLNODS(2,:),ILNODS(2,:));
% V3=setdiff(INTLNODS(3,:),ILNODS(3,:));
% GMANOD=[V1';V2';V3'];
% GMANOD=sort(GMANOD);
% GMANOD=unique(GMANOD);
