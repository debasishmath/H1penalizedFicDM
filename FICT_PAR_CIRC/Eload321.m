...
for i=1:3,
 ...
 for i=1:7,
     ...
     PHI(3)=ALAM3;
    for j=1:3,
        %F(j,1)=F(j,1)+PHI(j)*W(i)*1;
        
%         F(j,1)=F(j,1)+PHI(j)*W(i)* sin(pi*(ALAM1*X1+ALAM2*X2+ALAM3*X3))*...
%             cos(pi*(ALAM1*Y1+ALAM2*Y2+ALAM3*Y3) )*(1+pi*pi);
        F1(j,1)=F1(j,1)+PHI(j)*W(i)*(4-((ALAM1*X1+ALAM2*X2+ALAM3*X3-0.5)^2 + (ALAM1*Y1+ALAM2*Y2+ALAM3*Y3-0.5)^2-1/16));
    end
end
for i=1:3,
    F1(i,1)=F1(i,1)*DET*0.5;
end
